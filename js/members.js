...
const MEMBERS = [
    ...
    },
    {
        ...
        image: 'image: 'assets/daniel-nehemia.jpeg',',
        skills: [],
        socialLinks: {},
        gallery: [],
        courses: [],
...
// Ensure DATA and Utils are available
...
// Populate DATA.members after DATA is defined
...
function createMemberCard(member) {
    ...
    card.innerHTML = `
        <div class="member-image">
            <img src="${member.image}" alt="${window.Utils.getLangText(member.name, lang)}">
        </div>
        <div class="member-info">
            <h3>${window.Utils.getLangText(member.name, lang)}</h3>
            <p>${window.Utils.getLangText(member.role, lang)}</p>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!window.location.pathname.includes('index.html')) {
        console.log('Skipping members grid rendering');
        return;
    }
    ...
    if (membersGrid) {
        // Render members if grid exists
        const membersToRender = window.DATA.members || [];
        ...
        if (membersToRender.length > 0) {
            membersToRender.forEach(member => {
                try {
                    ...
                    console.error(`Error creating member card for ${member.name}: ${error.message}`);
                }
            });
        } else {
            console.warn('No members found to render');
        }
    } else {
        console.log('Members grid element not found');
    }
});
...
function renderMemberCourses(memberId) {
    ...
    // Find courses taught by this member
    const memberCourses = window.COURSES.filter(course => 
        course.teachers.includes(memberId)
    );

    // If no courses, hide the courses section
    if (memberCourses.length === 0) {
        const coursesSection = document.querySelector('.member-courses');
        if (coursesSection) coursesSection.style.display = 'none';
        return;
    }

    // Render courses
    const currentLanguage = document.documentElement.lang || 'he';
    memberCourses.forEach(course => {
        const courseElement = document.createElement('div');
        courseElement.classList.add('course-card');
        
        const courseLink = document.createElement('a');
        courseLink.href = `course-item.html?id=${course.id}`;
        courseLink.classList.add('course-link');
        courseLink.target = '_blank';
        
        const courseTitle = document.createElement('h3');
        courseTitle.textContent = Utils.getLangText(course.name, currentLanguage);
