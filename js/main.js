...
function initMemberPage() {
    ...
    if (memberDescriptionElement) {
        memberDescriptionElement.textContent = Utils.getLangText(member.description, currentLang);
    }
    ...
    if (memberCoursesContainer) {
        ...
        if (memberCourses.length > 0) {
            memberCourses.forEach(course => {
                const courseCard = createMemberCourseCard(course, currentLang);
    ...
    window.toggleLanguage = function() {
        ...
        if (memberDescriptionElement) {
            memberDescriptionElement.textContent = Utils.getLangText(member.description, newLang);
        }
        ...
        if (memberCoursesContainer) {
            ...
            memberCourses.forEach(course => {
                const courseCard = createMemberCourseCard(course, newLang);
                memberCoursesContainer.appendChild(courseCard);
            });
        }
...
function createMemberCourseCard(course, lang) {
    ...
    courseDescription.textContent = course.shortDescription && course.shortDescription[lang] 
...
function renderMemberGallery(memberId) {
    ...
    memberGalleryItems.forEach(item => {
        ...
        imageWrapper.classList.add('gallery-item-image-wrapper');
        
        const imageElement = document.createElement('img');
        imageElement.src = item.src;  
        imageElement.alt = item.title[document.documentElement.lang] || 'Gallery Item';
        
        const overlayElement = document.createElement('div');
        overlayElement.classList.add('gallery-item-overlay');
        
        const titleElement = document.createElement('div');
        titleElement.classList.add('gallery-item-title');
        titleElement.textContent = item.title[document.documentElement.lang] || 'Untitled';
        ...
        // Add click event to navigate to gallery item page
        imageWrapper.addEventListener('click', () => {
            window.location.href = `gallery-item.html?id=${item.id}`;
        });
        
        galleryItemElement.appendChild(imageWrapper);
        memberGalleryContainer.appendChild(galleryItemElement);
    });
}
